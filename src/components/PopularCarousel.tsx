...
const ThumbPill = ({
  ...
  <button
    ...
    className={`shrink-0 transition-all duration-300 ${
      active
        ? "opacity-100 ring-2 ring-white/60 ring-offset-1 ring-offset-black/20"
            ...
            >
              ...
              <div className="absolute inset-0">
                <img
                  ...
                  src={book.cover || noImage}
                  alt=""
                  className="w-full h-full object-cover scale-110 brightness-80"
                  onError={(e) => {
                    e.currentTarget.src = noImage;
                  }}
                />
              ...
              <div className="relative h-full flex items-end">
                <div className="flex items-end gap-3 sm:gap-4 p-4 sm:p-5 w-full">
                  {/* Cover – now visible on mobile with glass effect */}
                  <Link
                    to={DETAIL_PAGE}
                    params={{ id: `${book.id}` }}
                    ...
                    className="shrink-0"
                  >
                    <div className="rounded-xl overflow-hidden backdrop-blur-sm bg-white/10 shadow-xl ring-1 ring-white/20 transition-transform duration-300 hover:scale-105">
                      <div className="w-[64px] h-[92px] sm:w-[90px] sm:h-[128px]">
                        <img
                          loading="lazy"
                          src={book.cover || noImage}
                          alt={book.judul}
                  ...
                  <div className="flex-1 min-w-0 pb-0.5 sm:pb-1">
                    ...
                    <div className="flex items-center gap-2 mb-1 sm:mb-1.5">
                      ...
                      {book.status && (
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${STATUS_COLOR[book.status.toLowerCase()] ?? "bg-gray-400"}`}
                        />
                      )}
                      {book.status && (
                    ...
                    <Link
                      ...
                      search={{ title: slugify(book.judul) }}
                    >
                      <h2 className="text-base sm:text-lg font-bold text-white leading-snug line-clamp-2 hover:text-white/80 transition-colors">
                        {book.judul}
                      </h2>
                    </Link>
