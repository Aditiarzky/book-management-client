    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="flex flex-col gap-3 p-5 border-b border-gray-100 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {pagination.total.toLocaleString("id-ID")} buku terdaftar
          </p>
        </div>
      ...
      <div className="overflow-x-auto">
        {loading ? (
          ...
          <EmptyState
            message={
              searchTerm ? "Tidak ada buku yang cocok" : "Belum ada buku"
            }
          />
        ) : (
              ...
              {books.map((book) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 sm:hidden">
                          <span
                            className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md border capitalize ${STATUS_STYLE[book.status?.toLowerCase() ?? ""] ?? "bg-gray-50 text-gray-500 border-gray-100"}`}
                          >
                            {book.status ?? "—"}
                          </span>
                        </div>
                  ...
                  <Td className="hidden sm:table-cell">
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-lg border capitalize ${STATUS_STYLE[book.status?.toLowerCase() ?? ""] ?? "bg-gray-50 text-gray-500 border-gray-100"}`}
                    >
                      {book.status ?? "—"}
                    </span>
                  </Td>
                  ...
                  <Td className="hidden lg:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {(book.genres ?? []).slice(0, 2).map((g) => (
                        <span
                          key={g.id}
                          className="text-[11px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded-md"
                        >
                          {g.nama}
                        </span>
                      ))}
